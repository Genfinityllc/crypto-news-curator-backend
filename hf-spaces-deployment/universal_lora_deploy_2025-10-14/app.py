...
Forces SD 1.5 + LoRA generation or complete failure
"""
import os
...
class PureLoRACoverGenerator:
    ...
    def force_load_pipeline(self):
        """FORCE load SD 1.5 pipeline - FAIL HARD if impossible"""
        import gc
        
        logger.info("🚀 FORCING SD 1.5 pipeline load...")
        ...
        logger.info(f"🚀 Loading SD 1.5 pipeline on {device} with {torch_dtype}")
        
        try:
            # Load SD 1.5 with memory optimizations
            ...
            logger.info("📦 SD 1.5 model loaded, moving to device...")
            ...
            logger.info("🧪 Testing SD 1.5 pipeline...")
            ...
            logger.info("✅ SD 1.5 Pipeline loaded and tested successfully")
            
        except Exception as e:
            logger.error(f"❌ SD 1.5 Pipeline loading FAILED: {str(e)}")
            logger.error(f"📊 Error details: {type(e).__name__}")
            logger.error(f"📍 Traceback: {traceback.format_exc()}")
            raise SystemExit(f"SD 1.5 pipeline is required for LoRA generation: {e}")
    ...
    def generate_with_universal_lora(self, client, style, title):
        ...
        if not self.pipeline:
            raise Exception("SD 1.5 Pipeline not available")
