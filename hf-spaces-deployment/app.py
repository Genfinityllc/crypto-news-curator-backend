    ...
    def create_enhanced_background(self, width, height, client, style):
        ...
        if style == "energy_fields":
            ...
            for i in range(30):
                ...
                # Draw node
                node_size = random.randint(8, 20)
                draw.ellipse([x-node_size, y-node_size, x+node_size, y+node_size], 
                           fill=client_colors['accent'])
                
                # Draw glow
                for r in range(node_size+5, node_size+15):
                    alpha = max(0, 50 - (r-node_size)*5)
                    glow_color = client_colors['primary'] + (alpha,)
                    glow_img = Image.new('RGBA', (width, height), (0, 0, 0, 0))
                    glow_draw = ImageDraw.Draw(glow_img)
                    glow_draw.ellipse([x-r, y-r, x+r, y+r], outline=glow_color)
            ...
            for i in range(len(nodes)):
                for j in range(i+1, min(i+4, len(nodes))):
                    if random.random() < 0.3:  # 30% chance to connect
                        draw.line([nodes[i], nodes[j]], fill=client_colors['secondary'], width=2)
        
        elif style == "particle_waves":
            # Create particle wave patterns
            for wave in range(5):
                y_offset = wave * height // 5
