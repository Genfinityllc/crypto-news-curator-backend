...
async function main() {
  console.log('🎨 Regenerating with actual logo symbol not text...\n');

  const oldPath = path.join(OUTPUT_DIR, '02_glass_logo_lime_pattern.png');
  ...
  // Emphasize using the provided logo IMAGE not text
  const prompt = `pure black void background, dramatic cinematic lighting with subtle highlights, photorealistic 3D environment, the provided cryptocurrency logo symbol rendered very LARGE in crystal clear glass material taking up significant center space, the logo icon rendered in transparent glass with realistic refractions and light bending, LARGE translucent 3D crystal glass human hands reaching dramatically into the frame from the edges grabbing toward the logo, hands made of clear crystal glass with realistic fingers, scattered chrome coins at various angles, lime green (#dbff03) flat rounded square pattern in the bottom left corner extending off frame, shallow depth of field, the logo crafted from premium glass material with volumetric lighting, cinematic composition, 8k resolution, ultra-detailed, Octane render, Cinema 4D quality, premium 3D CGI product render, absolutely NO TEXT no letters no words no typography`
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, '02_glass_hands_lime_pattern.png'), Buffer.from(img.data));
  console.log('✅ Saved: 02_glass_hands_lime_pattern.png');
}
