  ...
  constructor() {
    ...
    this.styles = {
      ...
      '32_editorial_collage': {
        ...
        prompt: (logoSymbol) => `A cover for a crypto and finance publication in ${STYLE_DNA}. BUILD the composition, hero, materials, treatment, and story EXACTLY from the SCENE described in the additional instructions — that scene is the primary brief; do NOT impose a generic torn-paper layout or the same elements every time. Reproduce the COMPLETE ${logoSymbol} logo EXACTLY as provided in the logo image — BOTH the icon/mark AND its full wordmark together as one complete lockup — appearing exactly ONCE, prominent and integrated into the hero at substantial size, rendered CLEAN, CRISP and fully LEGIBLE on a flat surface FACING the viewer that is WIDE enough to fit the entire logo including the wordmark (a banner, sign, nameplate, panel, or poster). Do NOT crop to just the icon, do NOT drop, shorten, or omit the wordmark, and do NOT warp, curve, distort, or misspell it; spell the wordmark EXACTLY as in the provided image. Never tiny, never glowing. Every distinct element appears once; no duplicates or mirrors. Absolutely NO text, words, numbers, or letters anywhere; any document or seal shows blank paper or emblem imagery only, never fake or gibberish writing. 8k, ultra-detailed, sharp, dramatic cinematic contrast, editorial finish.`
      },
