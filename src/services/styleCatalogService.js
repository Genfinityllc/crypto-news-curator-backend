...
class StyleCatalogService {
  constructor() {
    ...
    this.styles = {
      ...
      '31_glossy_plastic_outlined': {
        ...
        prompt: (logoSymbol) => `soft purple (#9b8ec4) studio gradient background with gentle lighting, photorealistic 3D environment, the provided ${logoSymbol} cryptocurrency logo symbol rendered in thick puffy inflated glossy plastic — each individual letter and icon element of the logo is separately rendered as its own 3D glossy plastic piece floating freely in the scene, warm orange-yellow glossy plastic with a contrasting purple glossy plastic outline on each individual character giving a layered candy-like appearance, strong specular highlights and reflections on the shiny plastic surfaces, soft studio lighting from above creating clean highlights and gentle shadows, the plastic has a high-gloss wet-look finish like a vinyl toy, floating {{3D_ELEMENTS}} in matching glossy plastic material scattered around, soft shadow beneath each logo element on the studio floor, clean commercial product photography aesthetic, CRITICAL: do NOT merge the logo into one solid block or plaque — each letter and symbol must be its own separate floating 3D object with the material applied individually, cinematic composition, 8k resolution, ultra-detailed, Octane render, Cinema 4D quality, premium 3D CGI product render, no text`
      },
      ...
      '32_matte_plastic': {
        ...
        prompt: (logoSymbol) => `bright warm yellow (#f5c518) studio gradient background with soft even lighting, photorealistic 3D environment, the provided ${logoSymbol} cryptocurrency logo symbol rendered in chunky solid 3D matte-to-slightly-glossy red (#e63946) plastic — each individual letter and icon element of the logo is separately rendered as its own 3D plastic piece floating freely in the scene, the plastic has subtle surface texture like injection-molded toy plastic, rounded soft edges with substantial depth and weight on each character, soft studio lighting creating clean diffused highlights and gentle ambient shadows, each logo element casts its own soft shadow on the studio floor, floating {{3D_ELEMENTS}} in matching solid plastic material, clean minimal toy-like commercial aesthetic, product photography style, CRITICAL: do NOT merge the logo into one solid block or plaque — each letter and symbol must be its own separate floating 3D object with the material applied individually, cinematic composition, 8k resolution, ultra-detailed, Octane render, Cinema 4D quality, premium 3D CGI product render, no text`
      },
      ...
      '34_glass_color_fill': {
        ...
        prompt: (logoSymbol) => `dark charcoal grey (#2a2a2a) background with subtle gradient, soft cinematic studio lighting, photorealistic 3D environment, the provided ${logoSymbol} cryptocurrency logo symbol rendered with a clear transparent glass outer shell layered over a vivid red (#cc0000) opaque color fill underneath — each individual letter and icon element of the logo is separately rendered as its own 3D glass-over-color piece floating freely in the scene, the glass surface on each character is smooth glossy and reflective with bright specular highlights, the red color beneath the glass is rich and saturated visible through the transparent layer creating a candy-like depth effect, soft rounded edges with substantial thickness on each individual letter, floating {{3D_ELEMENTS}} in matching glass-over-color material, soft ambient lighting with clean reflections, CRITICAL: do NOT merge the logo into one solid block or plaque — each letter and symbol must be its own separate floating 3D object with the glass-over-color material applied individually, cinematic composition, 8k resolution, ultra-detailed, Octane render, Cinema 4D quality, premium 3D CGI product render, no text`
      },
      ...
      '36_neon_core_glass': {
        ...
        prompt: (logoSymbol) => `dark industrial concrete wall background with subtle metal panel texture and small rivets, moody cinematic lighting, photorealistic 3D environment, the provided ${logoSymbol} cryptocurrency logo symbol where each individual letter and icon element is rendered as its own separate 3D piece — each piece has a dark smoky translucent glass outer shell with a brilliant intense neon red-orange (#ff3300) inner core glow radiating outward from inside, the neon light bleeds through the dark glass creating a hot molten core effect, thick rounded puffy 3D form on each character with smooth glass surfaces showing subtle reflections of the environment, strong neon light casting warm red-orange glow and reflections onto the concrete wall behind and the glossy floor beneath, each logo element floats freely as its own 3D object, floating {{3D_ELEMENTS}} in matching dark glass with inner glow scattered around, dramatic contrast between the dark environment and the intense neon core, CRITICAL: do NOT place the logo inside a container or frame — each letter and icon is its own separate floating 3D glass piece with neon core, cinematic composition, 8k resolution, ultra-detailed, Octane render, Cinema 4D quality, premium 3D CGI product render, no text`
      }
    };
  ...
  getStylePrompt(styleId, logoSymbol, colorOverrides = null, customSubject = null, logoOverrides = null, patternOverrides = null) {
    ...
    prompt += ', CRITICAL: the logo must float freely in the scene as a 3D object - absolutely NO rectangular frames, NO bounding boxes, NO square borders, NO card shapes, NO plaques, NO panels, NO glass screens, NO glass cards, NO rounded rectangle containers behind or around the logo, NO solid block behind the letters, NO unified slab or base connecting the letters - each letter and icon element of the logo is individually rendered as its own separate 3D piece in the specified material, floating independently with gaps between characters, NOT merged into one solid shape';
