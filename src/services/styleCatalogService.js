  ...
  constructor() {
    ...
    this.styles = {
      ...
      '32_editorial_collage': {
        ...
        prompt: (logoSymbol) => `A cover for a crypto and finance publication in ${STYLE_DNA}. BUILD the composition, hero, materials, treatment, and story EXACTLY from the SCENE described in the additional instructions — that scene is the primary brief; do NOT impose a generic torn-paper layout or the same elements every time. The ${logoSymbol} cryptocurrency logo appears exactly ONCE, prominent and integrated into the hero at substantial size, but rendered CLEANLY and CRISPLY, preserving its EXACT shape, mark, and wordmark from the provided logo image — as a bold flat screen-print or a clear stamp on a surface FACING the viewer; do NOT warp, curve, emboss on a rounded surface, melt, distort, or misspell the logo or its wordmark; never tiny and never glowing. Every distinct element appears once; no duplicates or mirrors. Absolutely NO text, words, numbers, or letters anywhere; any document or seal shows blank paper or emblem imagery only, never fake or gibberish writing. 8k, ultra-detailed, sharp, dramatic cinematic contrast, editorial finish.`
      },
