...
class StyleCatalogService {
  ...
  getStylePrompt(styleId, logoSymbol, colorOverrides = null, customSubject = null, logoOverrides = null, patternOverrides = null, palettesBySymbol = null) {
    ...
    if (COLLAGE_STYLE_IDS.has(styleId)) {
      if (centeredLogo) {
        prompt += ` COMPOSITION: this must stay the SAME immersive, full-bleed, story-driven torn-paper photographic collage as the signature style — layered ripped photo fragments, deep tactile texture, real dimensional objects and materials, halftone, editorial depth and bold accent colour edge to edge, with NO empty or sparse areas. The ONLY change is placement: compose the scene so the ${displaySymbol} brand's logo MARK reads as the central focal point, worked INTO the collage materials at the centre like the other objects and catching the same lighting and texture. Do NOT turn this into a logo lockup, title card, or centered icon-plus-wordmark; do NOT render a big separate brand wordmark; do NOT leave the centre plain or empty. Keep the full immersive richness and depth of the non-centered collage — simply make the logo mark the clear central hero within that scene.`;
      } else if (!conceptMode) {
