...
function buildArtDirectSystem(withText) {
  const textBlock = withText
    ? `- A small set of FACTUAL TEXT CLIPPINGS (3 to 5) pulled truthfully from the article, grouped in ONE tidy zone (a clean column or a single corner stack) on torn white or aged-newsprint scraps. Give EACH clipping a clear TITLE-and-SUBTEXT hierarchy: the key figure or word LARGE and BOLD, its label or descriptor in a smaller, lighter weight beneath or beside it (for example "$1.4 MILLION" large with "interest in cash" small under it, or "9 MILLION" large with "shares" under it). Commit to ONE cohesive typographic direction for the whole cover: EITHER (a) a VINTAGE FINANCIAL-DOCUMENT look — bold display/masthead serif for the big figures, typewriter and rubber-stamped serif for labels, engraved stock-certificate type, with accent-colour stamped boxes; OR (b) a BOLD MODERN POSTER look — heavy CONDENSED sans or slab display for the big figures with smaller condensed labels. Emphasize the 1 to 2 most important figures with an accent-colour marker underline, a highlight block behind the text, or a hand-drawn circle. Each clipping contains ONLY its exact provided phrase (with its title/subtext pairing) and is otherwise BLANK torn paper — NEVER add body text, article copy, continuation sentences, or any extra words under, beside, or around the clippings. Spell every phrase EXACTLY; no other text anywhere in the image.`
