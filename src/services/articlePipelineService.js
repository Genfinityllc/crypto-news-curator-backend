...
function buildArtDirectSystem(withText) {
  const textBlock = withText
    ? `- FACTUAL TEXT CLIPPINGS (3 to 5) pulled truthfully from the article, rendered as a RICH editorial type treatment with STRONG hierarchy and a MIX of fonts, weights and sizes within ONE cohesive family — NEVER a uniform stack of same-size, same-font clippings. Make ONE clip the dominant HEADLINE, dramatically LARGER and bolder than the rest (the single most striking figure or phrase); the others are clearly smaller supporting clips. On data clips pair a BIG bold figure with a SMALLER label directly under or beside it (title-and-subtext), e.g. "$18B" large over "Robinhood volume" small, or "9 MILLION" over "shares". Commit to ONE direction but vary the treatment across clips like the reference covers: EITHER (a) VINTAGE FINANCIAL-DOCUMENT — a big display/masthead serif headline, with typewriter serif, rubber-stamped serif inside outlined boxes, and engraved stock-certificate type for labels; OR (b) BOLD INVESTIGATIVE POSTER — a HUGE condensed slab/sans display headline, with smaller condensed labels. On the 1 to 2 most important figures add an accent-colour MARKER UNDERLINE, a solid HIGHLIGHT BLOCK behind the text, a hand-drawn CIRCLE, or a red STAMPED BOX. Place the clips with clear hierarchy and dynamic sizing (headline prominent, the rest grouped legibly nearby), each on its own torn scrap. Each clip shows ONLY its exact provided phrase on otherwise-BLANK paper — NEVER add body text, article copy, continuation sentences, or any extra words. Spell every phrase EXACTLY; no other text anywhere in the image.`
...
- CINEMATIC and DIMENSIONAL, not flat: the cover is a collage of photographic cut-outs, but the HERO and key subjects have REAL depth, dramatic directional lighting, strong shadows and highlights, and rich physical texture (weathered metal, cracked stone, glass, fur, skin, aged currency) — like a dramatic photo composite, NEVER a flat, evenly-lit sticker. Keep it PHOTOGRAPHIC and gritty: NO clean vector illustration, NO glossy CGI product render, and absolutely NO neon, glow, holographic, wireframe, circuit, flowing-code, data-stream, or digital sci-fi look.
...
- The crypto logo is a PROMINENT, clearly visible part of the hero at substantial size, but always rendered CLEAN, CRISP, and LEGIBLE — preserve its EXACT mark and wordmark from the provided logo image. Place it as a bold FLAT screen-print or a clear stamp on a surface FACING the viewer (a poster, a flat stamped plate, a panel, a paper sheet), NOT warped, curved, or embossed around a rounded or angled 3D surface where the wordmark would distort. Never tiny, never omitted, never glowing.
...
Return: image_prompt (the full paragraph), focal_subject (the hero, 2-5 words), supporting_subjects (0 to 2 concrete objects), logo_treatment (how the logo integrates, prominent, kept clean, flat and legible on a viewer-facing surface, never warped or distorted), text_elements (${textElementsNote}), and accent1, accent2 (two bold saturated hex colours).`;
}
