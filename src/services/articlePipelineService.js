...
const SUBJECT_INSTRUCTIONS =
  'From the crypto news article, return ONE very short subject (2 to 4 words) naming a PHYSICAL OBJECT that can be sculpted in glass for the small floating 3D elements on a cover. It MUST be a tangible object (vault, chart, padlock, shield, bridge, rings, nodes, cubes, key, scales, rocket, ladder), never an abstract concept or activity (do NOT return things like "institutional staking" or "custody agreement"), and NEVER a gear, cog, or clockwork part. Depict what the NEWS is about, NOT the cryptocurrency itself, since its logo is already shown; do not name any coin or token. At most one adjective. Map the theme to an object: staking or custody -> "glass vault"; security -> "glass padlock"; ETF inflows or price -> "rising glass chart"; partnership -> "interlocking glass rings"; upgrade -> "ascending glass steps"; scaling -> "glass bridge". Keep it simple; the cover cannot handle a busy subject.';
...
const VISUAL_CONCEPT_INSTRUCTIONS = `You are the art director for a crypto and finance publication, designing ONE bold torn-paper collage cover from a news HEADLINE (you do not have the article body). The best covers tell the story through a SINGLE striking, often surreal, visual metaphor where the elements INTERACT, not a set of cutouts placed side by side. Examples of interaction: a whale sealed inside a glass jar, a bull smashing through a bank-vault door, a coin driving a money-printing press, a coin leaning against a crumbling wall, a hand squeezing coins until they crack. (Never use gears, cogs, or clockwork.)
...
- logo_treatment: one short phrase for how the crypto logo participates in the scene as a PROMINENT, clearly visible part of the hero at substantial size, never a small background detail (for example "stamped large on the printing press's plate", "minted large on the coin against the wall", "the vault's lock").

Keep it to ONE strong idea. Never describe a busy board of many separate items. No text or lettering in the scene.

AESTHETIC CONSTRAINT (critical): the scene must work as a FLAT, gritty, black-and-white PHOTOGRAPHIC torn-paper collage with halftone texture. Use ONLY physical, tangible, real-world subjects (animals, people, buildings, machines, presses, coins, hands, vaults, objects). Do NOT propose glowing, neon, holographic, wireframe, circuit-board, "data stream", flowing-code, sci-fi, or 3D-digital imagery, and NEVER use clichéd GEARS, COGS, or clockwork machinery as a metaphor, and the logo must NEVER glow — it is a flat printed mark or a matte minted coin. Translate any abstract or digital idea into a concrete physical metaphor.`;
...
- ONE bold, often surreal, PHYSICAL metaphor built around a single dominant HERO SUBJECT that the whole cover is about: a character, animal, person, object, coin, or machine placed in one symbolic situation. This can be an ACTION (a hand squeezing coins until they crack; a printing press stamping paper ledgers) OR a striking STATIC scene (a coin embedded in cracked concrete; a whale sealed inside a glass jar; a coin leaning against a crumbling wall; a Shiba Inu behind bars; a lone weathered portrait). The point is ONE clear hero the whole cover is about, NEVER a board of separate cut-outs placed side by side.
- BUILD DEEP, LAYERED CONCEPTS by constructing the metaphor's elements OUT OF meaningful materials, so each part is both a shape AND a substance tied to the story: for example prison bars MADE OF rolled banknotes or stock certificates, a wall BUILT from shredded filings, a machine welded from ledger pages, a coin struck from compressed documents, a nest woven from shredded contracts, a road paved with receipts. Do not settle for a generic object when the material its parts are made of can carry the article's meaning. Layer this into the hero and the supporting fragments.
- VARY THE HERO TYPE AND RENDERING STYLE across covers — do NOT always use the same photographic object. Choose whichever best fits THIS story: (1) a PHOTOGRAPHIC realistic subject (an animal, a person, a machine, a building) in a surreal situation; (2) a WEATHERED, textured symbolic OBJECT (a coin, a vault, a wheel, a book) with real depth; (3) a metallic or crystal RENDER of a coin or emblem; (4) a bold FLAT high-contrast GRAPHIC or heavy-HALFTONE icon (a cracked circle, a stark symbol) that is graphic, not photographic; (5) a black SILHOUETTE with graphic or halftone imagery inside it (a head in profile with a chart inside). Types 1 to 3 are DIMENSIONAL with dramatic directional lighting, strong shadows and rich texture; types 4 to 5 are bold, stark, high-contrast flat graphics. Whatever the type, make it striking and dramatic — NEVER a weak, flat, evenly-lit generic illustration, and absolutely NO neon, glow, holographic, wireframe, circuit, flowing-code, data-stream, or digital sci-fi look, and absolutely NO gears, cogs, cogwheels, clockwork, or machine-gear imagery anywhere (do not use a gear/cog as a metaphor for "upgrade", "mechanism", "network", or "work" — pick a concrete non-gear metaphor instead). Also do NOT default to server racks, server rooms, data centers, glowing screens, or a hooded hacker at a keyboard — use those ONLY when the article is specifically about data-center infrastructure or hacking/security. And do NOT fill the air with fog, haze, or smoke as a default atmosphere; keep the air clean unless the story is literally about smoke or fire.
...
- THE COLLAGE IS MADE OF TORN-EDGED PHOTOGRAPHS, NOT BLANK PAPER. Build the cover from torn / ripped-edged PHOTOGRAPHIC FRAGMENTS — each fragment a real photographic image directly RELEVANT to THIS story (a building, a road, a factory, a crowd, a machine, a coin, a chart, a portrait, a document, a hand) with a rough torn edge — layered into ONE cohesive scene around the dominant hero, together with the two bold accent-colour fields. The ripped edges are edges of PHOTOS. Do NOT use blank, empty, or plain paper scraps as background filler; every torn fragment shows a meaningful, story-relevant image.
...
- SECONDARY BRAND LOGOS (only when the input lists other brands from our library): you MAY weave in at most one or two as SUBTLE, small supporting details, each CREATIVELY integrated into the material of an object already in the scene (embossed on a coin, printed on a document, a sign, a screen, a badge, or worn into a torn photo fragment) — part of the collage's texture, NEVER a flat sticker slapped on top, NEVER competing with the hero, NEVER large or eye-catching. The single brand hero stays dominant; skip them entirely if none fit naturally.
...
async function artDirect(a = {}) {
  ...
  try {
    ...
    // Auto-detect OTHER brands from our logo library that this story names, so
    // the art director can weave them in subtly and creatively (never the hero).
    let brandsLine = '';
    try {
      const { detectBrands } = require('../data/brandLibrary');
      const detected = detectBrands(`${a.title || ''}\n${String(a.body || '')}`, [a.logoSymbol].filter(Boolean), 3);
      if (detected.length) {
        brandsLine = `\n\nOTHER BRANDS from our logo library that this story names — weave AT MOST one or two of the MOST relevant into the scene, integrated CREATIVELY into the material of an object (printed/embossed on a coin, document, sign, badge, screen, banner, or torn photo fragment) as a SMALL, SUBTLE mid-ground or background detail: ${detected.join(', ')}. These are supporting details ONLY — never the hero, never large, never eye-catching, and NEVER a flat logo pasted on top. If none fit the scene naturally, use none.`;
      }
    ...
    const r = await callResponses({
      ...
      input: `${src}${subj}${logo}${brandsLine}${factLines}`,
