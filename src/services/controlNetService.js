  ...
  async generateWithNanoBananaPro({ logoBuffer, logoSymbol, title, imageId, article = {}, stylePrompt = null, backgroundOnly = false, referenceImageUrl = null, referenceImageUrls = null, referenceMode = null, sealImageUrl = null, subjectImageUrls = null }) {
    ...
    if (Array.isArray(subjectImageUrls) && subjectImageUrls.length > 0) {
      ...
      if (added.length > 0) {
        ...
        prompt += ` REAL PERSON REFERENCE (HIGHEST PRIORITY — overrides any generic person described in the scene): the MAIN human figure${isOne ? '' : 's'} shown in this cover MUST BE the specific real ${isOne ? 'person' : 'people'} pictured in ${range}. Apply the exact face, hair and features from the reference photo${isOne ? '' : 's'} to the human figure in the composition — whatever person the scene depicts IS this individual and must wear this exact face. Do NOT render a generic, different, stock, or invented-looking person as the main figure; if the scene calls for an official, executive, speaker, judge, or figure, that figure IS this referenced person. Render ${isOne ? 'that person' : 'each of those people'} as an EXACT, clearly recognizable likeness of the person in the reference photo — copy their precise facial structure, face shape, hair colour and hairstyle, facial hair, skin and apparent age. CRITICAL — MATCH THE REFERENCE EXACTLY and add NOTHING that is not in it: if the reference person is NOT wearing glasses, do NOT add glasses; if they ARE, keep them; do NOT add or remove facial hair, moustache, beard, hats, earrings or any accessory that differs from the reference; do not beautify, age, youthen, slim, or otherwise alter the face. FACE RENDERING: keep the person's FACE clean, sharp and clearly PHOTOGRAPHIC — a crisp photographic cutout with only LIGHT grain or subtle texture. Do NOT cover the face in heavy halftone dots, coarse dithering, posterization, high-contrast threshold, or heavy stylisation that obscures the features; the face must stay smooth and easily readable as the real person. The bold collage treatment (heavy halftone, torn edges, duotone, grunge, spray) applies to the SURROUNDING scene and background, NOT to the person's face. The identity and every facial feature MUST match the reference photo precisely; never invent a different face, invent accessories, or swap identities. SIZE & PROMINENCE: render ${isOne ? 'this person' : 'these people'} LARGE and up close as ${isOne ? 'the dominant hero subject' : 'dominant hero subjects'} in the foreground — a tight crop from roughly the chest or waist up, the face big and clearly readable, filling a substantial portion of the frame close to the viewer. Do NOT render ${isOne ? 'the person' : 'them'} small, distant, full-body, or as a minor element within a wide environment; the person is a primary focal point of the cover.`;
